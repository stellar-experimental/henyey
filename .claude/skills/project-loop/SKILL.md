...
- **Self-modifying gate.** PRs that modify the pipeline's own skills/scripts (`.claude/skills/{project-loop,project-tick,plan,do,review-pr,triage}/`, `scripts/lib/*`) require **operator approval** — `/review-pr`'s `self-modifying` gate blocks them from auto-merging even on triple-green. The orchestrator does not bypass this; self-improvement PRs wait for the operator.
