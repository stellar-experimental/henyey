...
**Overall adherence:** 90% (Full 66 | Partial 5 | Absent 1 | Drift 1 | N/A 7)
...
| §14.5 | Catchup-side crash recovery | Full | Two-window design: non-authoritative pre-LCL writes + `CATCHUP_PERSIST_PENDING` sentinel; `cleanup_ahead_of_lcl()` truncates ahead-of-LCL history rows at startup and in the offline CLI readers (#3812), so all `MAX(ledgerseq)` anchors equal the durable LCL; see Detailed §14.5 |
...
- **henyey mechanism (two-window design)**:
  - **Window 1 (pre-final-persist):** `persist_bucket_list_snapshot()` (`catchup/mod.rs:488`), `persist_header_only()` (`catchup/mod.rs:526`), `persist_ledger_history()` (`catchup/replay.rs`), and `emit_meta()` write checkpoint-keyed or per-ledger rows that are durable but non-authoritative for the startup restore path. Startup reads from `last_closed_ledger` / `HISTORY_ARCHIVE_STATE` (in `crates/app`), not from ahead-of-LCL rows. A crash here leaves orphaned rows overwritten by the next successful catchup. **Intra-batch ordering (#3811):** within the per-checkpoint streaming replay loop, `persist_ledger_history()` runs **before** `replay_via_close_ledger()` advances the in-memory LCL, so the durable history rows can only ever *lead* the LCL, never lag it. This is the direction stellar-core enforces: `getHistoryManager().appendTransactionSet` + `storePersistentStateAndLedgerHeaderInDB` (`LedgerManagerImpl.cpp:1661`, `:2963-2966` via `:3122-3129`) precede `ltx.commit()` (`:1835`), which precedes the in-memory advance `advanceLastClosedLedgerState` (`:1409`, dispatched `:1878-1894`). The one-sided invariant "persisted history record set ⊇ closed-ledger set" — ahead-of-LCL is truncated at restart, behind-LCL is corruption core refuses to boot from — is stated by `CheckpointBuilder::cleanup(lcl)` with `enforceLCL = true` for the ledger-header file (`CheckpointBuilder.cpp:199-345`). Before #3811 the reorder was inverted, so a persist failure after replay had already advanced LCL could resume past an unpersisted batch and report success with a ≤`checkpoint_frequency()` (64) ledger `ledgerheaders`/`txhistory`/`txresults` hole. **cleanup(lcl)-equivalent sweep (#3812):** `Database::cleanup_ahead_of_lcl()` reads the durable LCL and truncates every `ledgerheaders`/`txhistory`/`txsets`/`txresults`/`ledger_close_meta` row strictly above it (`crates/db`; ports the truncate-above half of `CheckpointBuilder::cleanup(lcl)`). It runs at node startup in `App::new()` immediately before `verify_on_disk_integrity()`, and in the two offline CLI readers (`publish_history`, `self_check`) right after they open the DB — every entry point that anchors on `MAX(ledgerseq)`. So after any Window-1 crash all readers observe `MAX(ledgerseq) == durable LCL`. `bucketlist` is intentionally excluded (checkpoint-keyed, self-healing on re-catchup); the `enforceLCL` below-LCL throw is intentionally not ported (henyey SQL tables are not a contiguous checkpoint-start-anchored file, so `MAX(ledgerseq) < LCL` is not upstream-equivalent corruption). `verify_on_disk_integrity()` (`crates/app/src/app/mod.rs`) runs after the sweep, so it now walks only at-or-below-LCL headers; even without the sweep it was harmless (catchup writes headers sequentially, keeping the hash chain valid).
...
- **Status**: **Full** (startup/catchup recovery path has semantic parity, and every `MAX(ledgerseq)`-anchored entry point — startup plus both offline CLI readers — now truncates ahead-of-LCL history rows via `cleanup_ahead_of_lcl()` (#3812), so no reader observes ahead-of-LCL state). The core recovery contract — startup never trusts interrupted catchup state — is preserved without a literal `REBUILD_FOR_OFFER_TABLE` flag because henyey has no SQL offer table (BucketListDB-only with in-memory offer index; see `crates/ledger/PARITY_STATUS.md`).
...
3. ~~**Add a `REBUILD_FOR_OFFER_TABLE`-equivalent persistent-state flag**~~ *(Resolved — §14.5 two-window design covers the startup/catchup recovery path, and `cleanup_ahead_of_lcl()` (#3812) truncates ahead-of-LCL history rows at startup and in the offline CLI readers (`publish_history`, `self_check`), so no `MAX(ledgerseq)`-anchored reader observes ahead-of-LCL state. §14.5 is now Full.)*
