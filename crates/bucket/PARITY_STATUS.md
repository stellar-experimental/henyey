...
| `forgetUnreferencedBuckets()` | `retain_buckets()` | Full (mechanism diverges: set-based GC vs refcount; see §6 in Architectural Differences). `DISABLE_BUCKET_GC` kill-switch honored via `buckets.disable_bucket_gc` (#3153). |
...
6. **Bucket GC: set-based vs refcount-based (§8.2)**
   ...
   - **`DISABLE_BUCKET_GC` kill-switch (#3153)**: stellar-core gates GC on `!mConfig.DISABLE_BUCKET_GC` (`Config.h:590`, default `false` at `Config.cpp:202`) at two granularities — an early return in `cleanupStaleFiles()` (`BucketManager.cpp:896`) and a per-file guard inside `forgetUnreferencedBuckets()` (`:987`). henyey mirrors this with a single `App::bucket_gc_enabled()` early-return gate (== `!config.buckets.disable_bucket_gc`) at both GC entry points — `cleanup_stale_bucket_files_background()` (per-close path) and the catchup post-`assume_state` cleanup block — before any blocking task spawns. Because henyey's `retain_buckets()` is one atomic set-membership call rather than a per-bucket loop, the coarser single gate is the idiomatic mirror; the observable result (no on-disk bucket files deleted when disabled) is identical. The native TOML key is snake_case `buckets.disable_bucket_gc`; the UPPER_SNAKE `DISABLE_BUCKET_GC` is recognized in the compat-config translation path.
