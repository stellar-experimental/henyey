...
#
# Tests (mocked `gh` via PATH, real `flock` on a tmp lockfile):
#   1. test_flock_held_blocks_second_acquire
#        A live tick holds the issue's flock in a background subshell; a second
#        acquire for the same issue MUST exit 1, post no sentinel, dispatch
#        nothing. This is the exact #2917 incident — fails on origin/main
#        (60s-window returns "won"), passes post-fix (flock held for the whole
#        dispatch regardless of elapsed wall-time).
...
#   - records every invocation (one line per call) to $1/gh-calls.log
...
lock_path_for() {
  local issue="$1"
  echo "$HOME/data/$SESSION_ID/tick-locks/$issue.lock"
}
...
test_flock_held_blocks_second_acquire() {
  ...
  local SESSION_ID="sess-t1-$$"
  local issue=4242
  local lockfile
  lockfile="$(HOME="$tmpdir/home" SESSION_ID="$SESSION_ID" bash -c 'echo "$HOME/data/'"$SESSION_ID"'/tick-locks/'"$issue"'.lock"')"
  mkdir -p "$(dirname "$lockfile")"

  # A live tick holds the lock for the whole "dispatch". flock -n on the same
  # file from the acquire script must fail while this subshell lives.
  exec {hold_fd}>"$lockfile"
  flock -n "$hold_fd" || { echo "FAIL: $name — could not pre-hold lock"; FAILED=$((FAILED+1)); rm -rf "$tmpdir"; return; }

  local output exit_code
  output=$(PATH="$tmpdir:$PATH" HOME="$tmpdir/home" SESSION_ID="$SESSION_ID" \
...
test_acquire_succeeds_when_lock_free() {
  ...
  if ! grep -q "POST" "$tmpdir/gh-calls.log" 2>/dev/null; then
    ...
    echo "  gh calls: $(cat "$tmpdir/gh-calls.log" 2>/dev/null)"
    FAILED=$((FAILED + 1)); rm -rf "$tmpdir"; return
  fi
...
test_dead_holder_lock_is_acquirable() {
  ...
  local SESSION_ID="sess-t3-$$"
  local issue=4244
  local lockfile="$tmpdir/home/data/$SESSION_ID/tick-locks/$issue.lock"
  mkdir -p "$(dirname "$lockfile")"

  # Hold the lock in a background subshell, then kill it. When the process dies
  # the kernel closes the FD and releases the lock — no manual reap.
  ( exec {fd}>"$lockfile"; flock -n "$fd"; sleep 30 ) &
  local holder_pid=$!
  sleep 0.3
...
test_lost_race_writes_cooldown() {
  ...
  PATH="$tmpdir:$PATH" HOME="$tmpdir/home" SESSION_ID="$SESSION_ID" \
  ...
  if ! grep -qE "^$issue [0-9]+$" "$cooldown_file"; then
    ...
    echo "  contents: $(cat "$cooldown_file")"
    FAILED=$((FAILED + 1)); rm -f "$cooldown_file"; rm -rf "$tmpdir"; return
  fi

  echo "PASS: $name"
  PASSED=$((PASSED + 1))
  rm -f "$cooldown_file"; rm -rf "$tmpdir"
}
