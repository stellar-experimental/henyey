...
an adversarial audit (Fable model) **refuted the specific mechanism** I first proposed. This section is the
corrected version — see "Correction" below for what was wrong.
...
vs the short-probe (multiplier=60) max of ~1446–1510. So the 5-min max is **< 1240**, i.e. materially below
the 1-min number. **Magnitude is an UNPROVEN lower bound:** the long-probe search failed all the way down
and never passed a step, so the true sustainable rate D is only bounded (< 1240), not measured. The earlier
"≥17% / ~20% over-report" was over-stated — it compared a short-probe pass against a long-probe that never
passed.

**Root cause (corrected): backlog is DESTROYED during submission, not drained after it.** The binder is NOT
a post-submission drain budget. Two facts refute that:
...
- The classic tx-queue has finite admission capacity = `maxLedgerResources × TRANSACTION_QUEUE_SIZE_MULTIPLIER`
  ...
  `pending_depth` (4 ledgers) are **banned** (`tx_queue/mod.rs:43`). When offer R > drain D, the queue
  backlog grows during submission; once it exceeds capacity (and/or txs age past pending-depth) the excess
  is **evicted and banned** → those txs never apply → `check_accounts_synced` (`loadgen.rs:910`) can never
  reach 100% → the run fails regardless of any wait budget. Time-to-overflow ≈ capacity/(R−D), which is
  shorter at higher R — matching the observed order (1360 failed fastest, 1240 slowest). One fitted D
  ≈ 1120 reproduces all three failure times and the short-probe pass; but that fit is illustrative, not
  measured.
...
- **Direction is inherent to the test definition, present in stellar-core too** (shared harness; core also
  ...
  *fails the run instantly on the first non-PENDING submit* (`LoadGenerator.cpp:953-963`) whereas henyey
  ...
  1446–1510 vs 1531 absolute comparison. **Relative A/B results at the same window remain valid** (e.g.
  #3691 +6.4%).
- **Correct measurement fix (open-loop, not budget-scaling):** do NOT binary-search on 100 % backlog
  ...
  the wait budget with the window (my original suggestion) **would NOT work** — evicted/banned backlog
...
- Contradicts the skill's "short-probe reproduces the canonical ceiling within ~2%" claim — the real gap is
  larger (unquantified, but the long probe failed ≥ ~1240 vs short ~1490).

**Correction (2026-07-02, after Fable adversarial audit):** my first write-up called the root cause "a fixed
20-ledger post-submission drain budget, proven from code, definitive." That was WRONG: (1) the observed
failures were mid-submission (~222 s < 300 s submit time), so the post-submission timeout never triggered;
(2) that model predicts a 2.0× short/long ceiling ratio (167 % over-report), contradicting my own "~17 %"
number — model and magnitude were mutually exclusive. The actual binder is queue-capacity + pending-depth
ban during submission. Lesson: I over-fit a plausible-sounding code path and labelled it "definitive"
