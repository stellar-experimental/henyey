...
- → Refute pass: the skeptic reads `stellar-core/` and the plan, finds protocol 24+ is the minimum supported (AGENTS.md), so the pre-protocol-26 fallback is dead code and out of scope → `REFUTED`.
