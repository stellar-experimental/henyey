...
Based on the issue's status, invoke the specialist skill **as a foreground Codex sub-agent** so its work stays in its own context window AND the parent waits for it:

- Call `multi_agent_v1.spawn_agent` with `agent_type: "worker"` and the prompt from the table below.
