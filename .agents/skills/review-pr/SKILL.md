...
- `.agents/skills/{project-loop,project-tick,plan,do,review-pr,triage}/...` (any file under those skill dirs)
...
IS_SELF_MOD=$(echo "$SELF_MOD_FILES" | grep -Eq \
  '^\.agents/skills/(project-loop|project-tick|plan|do|review-pr|triage)/|^scripts/lib/(review-pr-merge|review-pr-verdicts|agent-worktree-contract|pipeline-anomaly-log)\.sh$|^\.github/skills/shared/scripts/' \
...
- **Skill files still parse**: every changed `.agents/skills/*/SKILL.md` has valid YAML frontmatter (`name` / `description` / `argument-hint`) and is well-formed markdown — no truncated frontmatter, no broken fences.
