...
testnet-only guideline in AGENTS.md.
...
9. **Print a startup summary** and cron setup instructions:
   ...
   survives across Codex sessions), add this crontab line:
     3,23,43 * * * * cd /home/tomer/henyey-1 && Codex -p '/monitor-tick' >> /tmp/monitor-tick.log 2>&1

   Or, to run ticks inside this Codex session only:
...
(`.Codex/skills/monitor-tick/SKILL.md`). `/monitor-tick` is self-contained:
...
- **External cron** (recommended — survives across Codex sessions):
  ```
  */10 * * * * cd /home/tomer/henyey-1 && Codex -p '/monitor-tick' >> /tmp/monitor-tick.log 2>&1
  ```
- **Codex session loop** (dies when Codex exits):
  ```
